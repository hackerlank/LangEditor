...
public:
    std::list<CodeNode* > childs; //  
    typedef std::list<CodeNode* >::iterator CHILDS_ITER;
    std::string nodeName; // 
	Properties propies; //
	...
	void  remove(CodeNode *child)
	{
		for (CHILDS_ITER iter = childs.begin(); iter != childs.end();){
            CodeNode* node = *iter;
			if (node == child)
			{
	...
	void addFirstChild(CodeNode *child)
	{
		childs.push_front(child);
	}
	bool insertChild(CodeNode *at,CodeNode *child)
	{
		for (CHILDS_ITER iter = childs.begin(); iter != childs.end();++iter){
            CodeNode* node = *iter;
			if (node == at)
...
#if(1)
	bool addNext(CodeNode *child)
	{
		if (parent){
			parent->insertChild(this,child);
			return true;
		}
		return false;
	}
	...
	CodeNode *parent;
	std::string getDescInfo()
	{
		std::stringstream info;
		info << " (";
		for (std::map<std::string,std::string>::iterator iter =propies.propies.begin(); iter != propies.propies.end();)
		{
			if (iter->second != "")
	...
	std::string toString(int x = 0)
	{
		std::stringstream codeSS;
		codeSS<<toSpace(x)<< nodeName << getDescInfo();
		if (childs.size())
		{
			codeSS << "{\n";
		}
	...
	int thisx;
	int thisy;
	int show(LangShow *showL,int x,int y)
	{
		thisx = x;
		thisy = y;
		showL->show(this,x,y);
		y += 45;
		CodeNode * oldNode = this;
		CodeNode * newNode = NULL; 
		for (CHILDS_ITER iter = childs.begin(); iter != childs.end();++iter){
            CodeNode * node = *iter;
            newNode = node;
			node->parent = this;
			int oy = node->show(showL,x+50,y);
			y = oy;
			showL->showRelation(this,oldNode,newNode);
			oldNode = newNode;
		}
		return y;
	}
	bool check(LangShow *showL,int x,int y)
	{
		return showL->check(this,x,y);
	}
	CodeNode *pick(LangShow *showL,int x,int y)
	{
		if (check(showL,x,y)) return this;
		for (CHILDS_ITER iter = childs.begin(); iter != childs.end();++iter){
           CodeNode *node = (*iter)->pick(showL,x,y);
		   if (node) return node;
		}
		return NULL;
	}
